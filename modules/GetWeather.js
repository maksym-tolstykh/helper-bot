...
export async function GetWeatherShedule(bot) {
  ...
  const response = await axios.get(
    `https://api.openweathermap.org/data/2.5/onecall?lat=50.7723&lon=29.2383&exclude=minutely,hourly&appid=${process.env.API_WEATHER}&units=metric&lang=ua`
  );
...
export function GetWeatherWeek() {
  return new Promise(async (resolve, reject) => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const sunrise = new Date(data[i].sunrise * 1000).toLocaleTimeString(
        "uk-UA"
      );
      const sunset = new Date(data[i].sunset * 1000).toLocaleTimeString(
        "uk-UA"
      );
